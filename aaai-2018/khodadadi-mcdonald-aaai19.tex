...
linear. \citet{KimKoh2009} proposed a specialized primal-dual
...
graphs~\cite{HallacPark2017}. Our methodology is similar in flavor
to~\cite{HallacPark2017}, though it uses a different likelihood
...
consensus ADMM takes about 20 seconds. \attn{fix this discription}
...
\begin{figure}[tb]
  ...
  \caption{Left: The true (orange) and estimated standard deviation 
    ...
    obtained using linearized ADMM with $\lambda_s=0.1$ and two
    values of $\lambda_t$: $\lambda_t=5$ (blue) and
    $\lambda_t=100$ (green). Middle: Convergence speed of linearized
    and consensus ADMM. Right: MAE for four models: admm\_opt: the
    proposed model with optimal values of $\lambda_t$ and $\lambda_s$,
    admm\_temp: no spatial penalty, admm\_sp: no temporal
    penalty.\attn{Fix these}} \label{fig:true_fitted_var}
\end{figure}
...
models. \attn{What is this next sentence trying to say?}
Interestingly, the proposed model with optimal parameters 
...
Weather Forecasts\footnote{\url{https://www.ecmwf.int}}. 
the 2 meter temperature measured daily at 12 p.m from January 1, 1960
to December 24, 2010. 

\paragraph{Preprocessing}

\attn{Can we redo these but with, say, Indianapolis instead of
  Bloomington?} Examination of the time-series alone demonstrates
strong differences 
...
temperature of three cities: Bloomington (USA), San Diego (USA) and
Manaus (Brazil). The time-series of Bloomington and San Diego show
...
temperature of Bloomington after detrending using this method. 
...
the time-series of Bloomington obtained from out proposed model. For
...
these cycles can be modeled by a sinusoidal function for Bloomington,
