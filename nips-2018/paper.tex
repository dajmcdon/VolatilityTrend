...
\end{equation}
 ...
 where $||u||_1$ is the $\ell_1$ norm of the vector $u$. Here, $\beta$ is the vector of the free parameters of the model and there is one parameter per observation. The \textit{penalty matrix} is:
...
The first term in \eqref{eq:l1tf} penalizes large residuals while the second term encourages smooth estimated $\beta$. Specifically, with the penalty matrix \eqref{eq:d_matrix}, the estimated $\beta$ will be piecewise linear \citep{kim_$ell_1$_2009}. The trade-off between the two aforementioned objectives is determined by the constant $\lambda$. \citep{kim_$ell_1$_2009} proposed a specialized primal-dual interior point (PDIP) method for solving the optimization problem \eqref{eq:l1tf} with the penalty matrix \eqref{eq:d_matrix}.
